...
<div class="card w-full max-w-sm bg-base-100 px-4 py-8 shadow-xl">
  ...
  <form class="card-body pt-2">
    ...
    <div class="form-control mt-6"><a href="http://tolulope:3030/oauth/github" id="github" class="btn">Login with GitHub</a></div>
  </form>
</div>
</div>`
...
const showChat = async () => {
  ...
  const messages = await client.service('chats').find({
    query: {
      $sort: { createdAt: -1 },
      $limit: 25
    }
...
addEventListener('#send-message', 'submit', async (ev) => {
  ...
  await client.service('chats').create({
    text: input.value
  })
...
client.service('chats').on('created', addMessage)
